...
variable (n m w k s:ℕ )(hn :1 ≤ n)(hnk :k ≤ n)(hm :1 ≤ m ∧ m ≤ k )(hw : 1 ≤ w ∧ w ≤ n)(hs :1 ≤ s ∧ s ≤ k-1)(hk : 2 ≤ k)
...
lemma lemma1 {q a b : ℕ  } (ha : 1 ≤ a)(h1 : q ∣ a - 1) (h2 : q ∣ a * b) : q ∣ b := by
  have h_coprime : Nat.gcd a q =1 := by
    ...
    have h_dvd_a2  : d ∣ a-(a-1) := Nat.dvd_sub  h_dvd_a h_dvd_a1
...
lemma lemma3  {n a1 a2 :ℕ }(_: n ≥  1)  (ha1 : 1 ≤  a1 ∧ a1 ≤ n) (_: 1 ≤ a2 ∧ a2 ≤ n)
    (h : n ∣ a1 * (a2 - 1)) : Nat.gcd n a1∣ a1 ∧ (n / Nat.gcd n a1) ∣ (a2 - 1) := by
  -- 设置p和q的定义
  set p := Nat.gcd n a1
  set q := n / p
  set m := a1 / p
...
theorem ex15_results
  ...
  (hn :1 ≤ n)(hk :2 ≤ k)(hnk :k ≤ n)(hs :1 ≤ s ∧ s ≤ k-1):
 ¬ n ∣ (a k) * ((a 1)-1) := by
------------------------
  have h_1_C : 1 ∈ C k := by
    simp [C]
...
---------------------------
  have h_lemma3 := lemma3 hn  h_a1_le_n_pos  h_a2_le_n_pos  h_n_dvd
  set p := Nat.gcd n (a 1)
  set q := n / p
...
-- 重要的归纳证明环节
  ...
  -- 归纳步骤：从 j 到 j+1
  -- hj: 1 ≤ j
  -- h_ind: 你的归纳假设，即命题对 j 成立
  -- 你的目标是证明命题对 j+1 成立

    ---归纳法从1开始，该咋整呢？？？？
        --case one 不识别啊？？？？
        --i=1时有q|a2-1 159h_q_dvd_a2_sub_1
        --当i=2时有165 q_dvd q|a2*（a3-1）引用lemma1得q|a3-1
        --以此类推
        --case succ
        --从 q_dvd 得到 q ∣ a (i+1) * (a (i+2) - 1)
        --have h_q_dvd_mul : q ∣ (a (i+1)) * (a (i+2) - 1) := q_dvd (i+1) hi'
        -- 从归纳假设得到 q ∣ a (i+1) - 1
        -- have h_q_dvd_sub : q ∣ (a (i+1)) - 1 := ih hi'
        -- 应用 lemma1 得到 q ∣ a (i+2) - 1


    /-intro i hi
    induction i using Nat.le_induction with
  -- 基本情况：i = 1 (对应证明 q ∣ a 2 - 1)
    case one =>
      exact h_q_dvd_a2_sub_1

  -- 归纳步骤：i → i+1
    caase succ have hi' : i + 1 ∈ C k := by
      simp [C] at hi
      exact hi
    -- 从 q_dvd 得到 q ∣ a (i+1) * (a (i+2) - 1)
      have h_q_dvd_mul : q ∣ (a (i+1)) * (a (i+2) - 1) := q_dvd (i+1) hi'
    -- 从归纳假设得到 q ∣ a (i+1) - 1
      have h_q_dvd_sub : q ∣ (a (i+1)) - 1 := ih hi'
    -- 应用 lemma1 得到 q ∣ a (i+2) - 1
      have h_a_pos : 1 ≤ a (i+1) := by
        have h_mem : i+1 ∈ A k := by simp [A]; linarith [hi'.2]
        exact (h_range (i+1) h_mem).1
      exact lemma1 h_a_pos h_q_dvd_sub h_q_dvd_mul-/
...
------------------------------
  ...
  -- 现在我们有p | a 1和q | a k - 1
  have h_pq_dvd_a1_ak_sub_1 : (p*q)∣ (a 1) * ((a k)-1) := Nat.mul_dvd_mul h_p_dvd_a1 h_q_dvd_a_k_sub_1
...
------------------------
  ...
  -- 计算差值
    have h_diff : (n : ℤ) ∣ ((a 1 : ℤ) * (a k : ℤ) - (a 1 : ℤ)) - ((a k : ℤ) * (a 1 : ℤ) - (a k : ℤ)) :=
